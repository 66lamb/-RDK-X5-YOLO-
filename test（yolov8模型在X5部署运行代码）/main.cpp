...
#define MODEL_PATH "/root/rdk_model_zoo/demos/detect/YOLOv8/YOLOv8-Detect_YUV420SP/ptq_models/yolov8n_detect_bayese_640x640_nv12_modified.bin"

// 推理使用的测试图片路径
// Path of the test image used for inference.
#define TESR_IMG_PATH "/root/test/frame.jpg"
...
#define IMG_SAVE_PATH0 "/root/test/frame_out0.jpg"
#define IMG_SAVE_PATH1 "/root/test/frame_out1.jpg"
#define IMG_SAVE_PATH2 "/root/test/frame_out2.jpg"
// 模型的类别数量, 默认80
...
// YUV420转RGB24转换函数（BT.601标准）
void yuv420_to_rgb24(const uint8_t* yuv, uint8_t* rgb, int width, int height) {
    const int frameSize = width * height;
    const int chroma_w = width >> 1;
    const int chroma_h = height >> 1;

    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            const int Y = yuv[y * width + x];
            const int U = yuv[frameSize + (y >> 1) * chroma_w + (x >> 1)];
            const int V = yuv[frameSize + chroma_w * chroma_h + (y >> 1) * chroma_w + (x >> 1)];
...
bool convert_jpg_to_nv12(const cv::Mat& img, char*& nv12_data, int& nv12_size) {
    // 1. 读取JPG图像
    cv::Mat bgr_img = img;
    if (bgr_img.empty()) {
    ...
    // 3. 转换为YUV_I420格式（YUV420p）
    cv::Mat yuv_img;
    cv::cvtColor(bgr_img, yuv_img, cv::COLOR_BGR2YUV_I420);
 
    // 4. 计算NV12数据大小
    const int y_size = width * height;
    const int uv_size = y_size / 2;  // NV12的UV平面是交错存储的，大小为Y平面的一半
    nv12_size = y_size + uv_size;
 
    // 5. 动态分配NV12缓冲区
    nv12_data = new char[nv12_size];
    if (!nv12_data) {
        std::cerr << "Error: Failed to allocate memory for NV12 data" << std::endl;
        return false;
    }
 
    // 6. 重组像素格式为NV12
    // 复制Y平面
    memcpy(nv12_data, yuv_img.data, y_size);
 
    // 重组UV平面（I420的U和V是分开的，NV12需要交错存储）
    const uchar* u_plane = yuv_img.data + y_size;
    const uchar* v_plane = yuv_img.data + y_size + (y_size / 4);  // UV平面在I420中是分开的
    
    for (int i = 0; i < y_size / 4; ++i) {  // UV分量数量是Y分量的1/4
        nv12_data[y_size + 2 * i]     = u_plane[i];
        nv12_data[y_size + 2 * i + 1] = v_plane[i];
    }
...
int main(int argc, char** argv)
{
    ...
    if (sp_open_camera_v2(vio_object, 0, -1, 1, &parms, widths, heights) != 0) {
    ...
    if (sp_start_display(display_obj, 1, disp_w, disp_h) != 0) {
        printf("[Error] Failed to start display!\n");
    ...
    // 分配缓冲区
    int yuv_size = args.width * args.height * 3 / 2;
    uint8_t* yuv_buf = static_cast<uint8_t*>(malloc(yuv_size));
    uint8_t* rgb_buf = static_cast<uint8_t*>(malloc(args.width * args.height * 3));

    printf("实时捕获已启动，按 'q' 退出...\n");
   ...
   const int fps_update_interval = 1; // 每 1 帧计算一次 FPS
...
while (true) {
        ...
        if (sp_vio_get_frame(vio_object, reinterpret_cast<char*>(yuv_buf), args.width, args.height, 2000) != 0) {
            printf("捕获超时，重试中...\n");
            continue;
        }

        // 2. 转换为RGB
        yuv420_to_rgb24(yuv_buf, rgb_buf, args.width, args.height);
        
        std::cout<<"成功获取一张"<<std::endl;
       ...
       cv::Mat img(args.height, args.width, CV_8UC3, rgb_buf);
        ...
        cv::Mat resized_img;
        cv::resize(img, resized_img, cv::Size(384, 216));
        cv::imshow("Real-time Detection", resized_img);

        // 合并 waitKey 调用，避免多次阻塞
        int key = cv::waitKey(1);  // 返回 ASCII 码值

        if (key == 's') {          // 直接比较字符（C++ 会自动转换）
            std::cout << "已输出";
            
            
            if(i==0){
            cv::imwrite(IMG_SAVE_PATH0, img);}
            if(i==1){
            cv::imwrite(IMG_SAVE_PATH1, img);}
            if(i==2){
            cv::imwrite(IMG_SAVE_PATH2, img);}
            i++;
        }
        else if (key == 'q') {   // 使用 else if 避免多次判断
       ...
       // 计算时间差（转换为毫秒）
        ...
        double fps = fps_update_interval / (duration / 1000.0); // 转换为秒
